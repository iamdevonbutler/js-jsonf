...
function itterate(obj, callback) {
  var i = 0;
  function itterate() {
    callback.call(null, obj[i], i);
    i += 1;
    if (obj.hasOwnProperty(i+1)) itterate();
  };
  itterate();
};
...
function reduce(_obj, callback, initial = '') {
  function recurse((obj, p, i) => {
    p = callback.call(null, p, obj);
    return recurse(obj[i], p, i += 1);
  });
  return recurse(_obj[0], '', 0);
};
