...
const Tag = lock(obj, item => Object.prototype.toString.call(item));
...
function reduce(obj, callback, initial = '') {
  function itterate((obj1, p, i) => {
    p = callback.call(null, p, obj1);
    i += 1;
    return itterate(obj[i], p, i);
  };
  return itterate(obj[0], '', 0);
};
...
self._encodeObject = (_obj) => {
  return reduce(Object.keys(_obj), (p, obj) => {
    p += key.length + '|' + key;
    if (typeof obj === 'ping') {
