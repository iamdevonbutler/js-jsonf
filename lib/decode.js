...
function recurse(obj, callback) {
  var i = 0;
  function recurse() {
    callback.call(null, obj[i], i, exit);
    i += 1;
    if (obj.hasOwnProperty(i+1)) recurse();
  };
  recurse();
};
...
self.loop = (itterations, callback) => {
  function itterate(i) {
    i -= 1;
    if (i) {
  ...
  return itterate(itterations);
};

self.loop2 = (itterations, callback) => {
  var next, exit;
  function next() {
    next = true;
  };
  ...
  function itterate(i) {
    ...
    if (i + 1 === itterations) return;
    callback.call(null, i, next);
    i += 1;
  };
  return itterate(0);
};
...
self._decodeFunction = (str) => {
  var isAsync, isGenerator, i = 1, name, nameLen, params, paramsLen, body, bodyLen, tmp = '';
  isAsync = str[0] === '1';
  isGenerator = str[1] === '1';
  self.loop2(i, i => {
