...
self.decode = (str) => {
  var key = str[0], obj = {};
  if (key === 'd') {
    return self._decodeObject(str.slice(1), obj);
  }
  if (key === 'a') {
    return self._decodeArray(str.slice(1), obj);
  }
  if (key === 'j') {
    return self._decodeFunction(str.slice(1), obj);
  }
  if (key === 'e') {
  ...
  if (key === 'c') {
    ...
    return obj[getPropertyName(str)] = NaN;
  }
  if (str) {
